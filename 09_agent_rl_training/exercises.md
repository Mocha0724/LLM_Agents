...
4. SkyRL-Agent 的 async dispatch 解决了什么瓶颈？类比到普通 RL 训练任务，是否有同样问题？
...
3. **(系统)** 从零搭一个 GRPO 训练 stack，需要哪些组件（rollout / reward / inference / optimizer / 监控）？
4. **(开放)** RLVR 在「reward 难以验证」的任务（开放对话、创意写作）上失效，那么 *RLAIF* / *Argos-style verifier* 是出路吗？
5. **(业务)** *重定位成功率* 算可验证 reward 吗？尝试设计一个把 LLM agent 接入「定位 pipeline」并用 RLVR 训练的方案。
