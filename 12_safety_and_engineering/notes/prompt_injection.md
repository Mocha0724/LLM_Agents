...
## 4. 工具型 agent 的设计建议（通用）

凡是 LLM 会读取 *用户生成内容 / 第三方文本 / OCR 结果* 的应用，都是 IPI 的潜在入口。建议：

1. 让外部文本只作为 *显示数据* 透传给最终用户，尽量不进入 LLM 决策上下文；若必须，使用 spotlighting / tagging。
2. 对用户评论 / 网页摘要等做 *净化 + 信任标签* 后再喂给模型。
3. 任何 *写操作 / 不可撤销动作 / 涉及金钱或身份* 的步骤必须走 HITL。
