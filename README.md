...
本仓库不是又一份「调几行 LangChain API」的入门教程，而是一份系统化的学习材料：

- **理论与论文**：每一章先给出综述、关键论文笔记、思想演进图。
- **代码与实验**：每一章配 1-3 个最小可运行 Jupyter Notebook。
- **思考与面试题**：每章附思考题与面试题，便于复习与拓展。
- **业务桥接**：单设 `09_agent_rl_training` 与 `11_spatial_map_agents` 两条脉络，把 Agent 与空间/地图类业务结合。
...
- **业务实践（持续）**：11 + 99 项目 A，串联空间/地图类业务的端到端落地。
